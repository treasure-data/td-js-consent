...
class TdConsentForm extends LitElement {
  ...
  getInputs (contextId, name, inputName, status) {
    return [
      html`<label> ${this.getInput(contextId, name, inputName, status, 'given')} Yes </label>`,
      html`<label> ${this.getInput(contextId, name, inputName, status, 'rejected')} No </label>`
    ]
  }
  ...
  getInput (contextId, name, inputName, preferStatus, status) {
    if (preferStatus === status) {
      return html`
        <input type="radio" name=${inputName} data-purpose=${name} data-cid=${contextId} value=${String(status)} checked>
      `
    }

    return html`
      <input type="radio" name=${inputName} data-purpose=${name} data-cid=${contextId} value=${String(status)}>
    `
  }
}
